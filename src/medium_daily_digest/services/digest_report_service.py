...
from src.medium_daily_digest.models import DigestLink, DigestReport, EmailMessage
...
class DigestReportService:
    def __init__(self, gmail_reader: GmailReaderService | None = None) -> None:
    ...
    def build_report(self) -> DigestReport:
        ...
        links = self._extract_links_from_messages(email_messages)
        return DigestReport(
            markdown=self._build_markdown(links),
            html=self._build_html(links),
        )

    def build_terminal_markdown(self) -> str:
        return self.build_report().markdown

    def _build_markdown(self, links: list[DigestLink]) -> str:
        ...
        for link in links:
            ...
            lines.append(f"[Link para leitura]({link.url})")
            lines.append("")

        return "\n".join(lines).rstrip()

    def _build_html(self, links: list[DigestLink]) -> str:
        ...
        for link in links:
            article_blocks.append(
                ...
                f"<h2 style=\"margin: 0 0 8px 0;\">{escape(link.title)}</h2>"
                f"<p style=\"margin: 0;\"><a href=\"{escape(link.url, quote=True)}\">"
                "Link para leitura"
                "</a></p>"
                "</section>"
            )
